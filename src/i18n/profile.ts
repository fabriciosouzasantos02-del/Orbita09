import { Language } from './types';

export const profileTranslations: Record<Language, Record<string, string>> = {
  pt: {
    // Local settings keys
    "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.": "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.",
    "Dados de Nascimento e Orbe": "Dados de Nascimento e Orbe",
    "Dados atualizados com êxito! Recalculando coordenadas estelares...": "Dados atualizados com êxito! Recalculando coordenadas estelares...",
    "Data de Nascimento": "Data de Nascimento",
    "Horário exato": "Horário exato",
    "Atualizar e Recalcular Mapa": "Atualizar e Recalcular Mapa",
    "Preferências Globais": "Preferências Globais",
    "Idioma do Oráculo": "Idioma do Oráculo",
    "Autenticação Segura Local": "Autenticação Segura Local",
    "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.": "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.",

    currentLocation: 'Sua Configuração Natal',
    logout: 'Desconectar Sessão',
    unsupportedFirebase: 'Modo Local Ativo: Sua sessão e dados estão armazenados localmente de forma privada.',
    astroModelEngine: 'Motor Cósmico Interno: Precisão matemática offline.',

    // Billing / Premium
    sub_status: 'Status da Assinatura',
    sub_expires: 'Válido até',
    sub_billing_info: 'Faturamento via Stripe Subscription',
    sub_manage: 'Gerenciar Cobrança',
    sub_created_at: 'Membro desde',
    trial_remaining: 'Seu período de sintonia inicial gratuita está ativo de forma ilimitada.',
    sub_sim_pay_success: 'Acesso Premium renovado com sucesso via Stripe!',

    // Extra maps
    "Lista de Mapas Extras Cadastrados": "Lista de Mapas Extras Cadastrados",
    "Processando alinhamento estelar secundário...": "Processando alinhamento estelar secundário...",
    "Nenhum mapa extra cadastrado.": "Nenhum mapa extra cadastrado.",
    "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.": "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.",
    "Nome": "Nome",
    "Cidade": "Cidade",
    "Visualizar": "Visualizar",
    "Remover": "Remover",
    "Limites": "Limites de mapas extras atingido (máx. 2).",

    // Personal Protection and Edit Social Profile keys
    "Amuletos & Símbolos de Proteção Pessoais": "Amuletos & Símbolos de Proteção Pessoais",
    "Seu Elemento Ativo: Ar": "Seu Elemento Ativo: Ar",
    "Pedras de Filtro": "Pedras de Filtro",
    "Editar Dados do Perfil Social": "Editar Dados do Perfil Social",
    "Adicione suas mídias sociais e bio para os outros visualizadores.": "Adicione suas mídias sociais e bio para os outros visualizadores.",
    "Bio Curta": "Bio Curta",
    "Escreva algo curto sobre você...": "Escreva algo curto sobre você...",
    "Instagram (@usuario)": "Instagram (@usuario)",
    "@seu_perfil": "@seu_perfil",
    "Facebook (usuário ou link)": "Facebook (usuário ou link)",
    "link_do_facebook": "link_do_facebook",
    "Cancelar": "Cancelar",
    "Salvar Perfil": "Salvar Perfil",

    // Map Sync keys
    "MAPA SINCRONIZADO": "MAPA SINCRONIZADO",
    "Seu mapa astral principal foi protegido e sincronizado com sua conta.": "Seu mapa astral principal foi protegido e sincronizado com sua conta.",
    "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.": "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.",
    "SINCRONIZAÇÃO ATIVA": "SINCRONIZAÇÃO ATIVA",
    "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.": "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.",
    "SALVAR MAPA": "SALVAR MAPA",
    premium_benefit_1_title: 'Análise Avançada da IA Orbia',
    premium_benefit_1_desc: 'Conselhos diários e interpretação personalizada em tempo real baseada em seus trânsitos e mapa astral.',
    premium_benefit_2_title: 'Tarot Cósmico Profissional',
    premium_benefit_2_desc: 'Tiragens ilimitadas com correspondências alquímicas, astrológicas e numéricas exclusivas.',
    premium_benefit_3_title: 'Compatibilidade e Sinastria',
    premium_benefit_3_desc: 'Entenda os fios invisíveis de atração, desafios e afinidades entre sua energia e a de quem você ama.',
    premium_benefit_4_title: 'Oráculo dos Sonhos',
    premium_benefit_4_desc: 'Desvende as mensagens ocultas de seu subconsciente com interpretações guiadas e arquivamento seguro.',
    premium_benefit_5_title: 'Mapas Extras Ilimitados',
    premium_benefit_5_desc: 'Crie, salve e analise as mandalas astrais e relatórios numerológicos de seus amigos e familiares.',
    select_plan_title: 'Escolha seu Alinhamento',
    select_plan_desc: 'Selecione o ciclo ideal para continuar sua jornada cósmica:',
    plan_annual_save: 'Economize 33%',
    plan_annual_title: 'Plano Anual',
    plan_annual_sub: 'Cobrado anualmente',
    year: 'ano',
    month: 'mês',
    plan_monthly_title: 'Plano Mensal',
    plan_monthly_sub: 'Cancele quando quiser',
    plan_annual_desc_info: 'O ciclo anual garante 12 meses completos de sintonia astrológica ininterrupta, trânsitos diários e IA por menos do que o preço de um café por semana.',
    plan_monthly_desc_info: 'O ciclo mensal é cobrado automaticamente a cada 30 dias. Oferece total flexibilidade de cancelamento a qualquer momento diretamente no painel.',
    btn_connecting_stripe: 'CONECTANDO...',
    stripe_secure_payment_guaranteed: 'Stripe Secure Checkout • Ativação Imediata',
    conversion_nasa_text: 'Tecnologia astrológica avançada de interpretação, com cálculos astronômicos de altíssima precisão conectados ao motor softwares da NASA/JPL, calculando posições planetárias, casas e trânsitos em tempo real.'
  },
  en: {
    "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.": "Inspect the local cryptographic keys of your mystical birth chart and manage privacy settings.",
    "Dados de Nascimento e Orbe": "Birth Data and Orb",
    "Dados atualizados com êxito! Recalculando coordenadas estelares...": "Data successfully updated! Recalculating stellar coordinates...",
    "Data de Nascimento": "Birth Date",
    "Horário exato": "Exact Time",
    "Atualizar e Recalcular Mapa": "Update and Recalculate Chart",
    "Preferências Globais": "Global Preferences",
    "Idioma do Oráculo": "Oracle Language",
    "Autenticação Segura Local": "Secure Local Authentication",
    "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.": "Your dream journals, tarot oracles, and settings are saved solely in the device's local browser sandbox to maintain absolute privacy.",

    currentLocation: 'Your Natal Coordinates',
    logout: 'Log Out Session',
    unsupportedFirebase: 'Active Local Mode: Your session and data are stored locally and privately.',
    astroModelEngine: 'Internal Cosmic Engine: Mathematical precision offline.',

    // Billing / Premium
    sub_status: 'Subscription Status',
    sub_expires: 'Expiration Date',
    sub_billing_info: 'Billing via Stripe Subscription',
    sub_manage: 'Manage Billing',
    sub_created_at: 'Member since',
    trial_remaining: 'Your initial free synchronization is fully active with unlimited access.',
    sub_sim_pay_success: 'Premium access successfully activated via Stripe!',

    // Extra maps
    "Lista de Mapas Extras Cadastrados": "List of Registered Extra Charts",
    "Processando alinhamento estelar secundário...": "Processing secondary stellar alignment...",
    "Nenhum mapa extra cadastrado.": "No extra charts registered.",
    "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.": "Add up to 2 profiles of friends or family to compare astrological charts and synastries.",
    "Nome": "Name",
    "Cidade": "City",
    "Visualizar": "View",
    "Remover": "Remove",
    "Limites": "Extra charts limit reached (max 2).",

    // Personal Protection and Edit Social Profile keys
    "Amuletos & Símbolos de Proteção Pessoais": "Personal Protection Amulets & Symbols",
    "Seu Elemento Ativo: Ar": "Your Active Element: Air",
    "Pedras de Filtro": "Filter Stones",
    "Editar Dados do Perfil Social": "Edit Social Profile Data",
    "Adicione suas mídias sociais e bio para os outros visualizadores.": "Add your social media and bio for other viewers.",
    "Bio Curta": "Short Bio",
    "Escreva algo curto sobre você...": "Write something short about yourself...",
    "Instagram (@usuario)": "Instagram (@username)",
    "@seu_perfil": "@your_profile",
    "Facebook (usuário ou link)": "Facebook (username or link)",
    "link_do_facebook": "facebook_link",
    "Cancelar": "Cancel",
    "Salvar Perfil": "Save Profile",

    // Map Sync keys
    "MAPA SINCRONIZADO": "CHART SYNCHRONIZED",
    "Seu mapa astral principal foi protegido e sincronizado com sua conta.": "Your main birth chart has been protected and synchronized with your account.",
    "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.": "From now on you will receive continuous updates about transits, cycles, energy frequencies, and celestial movements related to your chart.",
    "SINCRONIZAÇÃO ATIVA": "ACTIVE SYNCHRONIZATION",
    "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.": "Save your chart to receive personalized updates on transits, energies, predictions, and astrological movements in real time.",
    "SALVAR MAPA": "SAVE CHART",
    premium_benefit_1_title: 'Advanced Orbia AI Analysis',
    premium_benefit_1_desc: 'Daily advice and real-time personalized interpretation based on your transits and birth chart.',
    premium_benefit_2_title: 'Professional Cosmic Tarot',
    premium_benefit_2_desc: 'Unlimited draws with exclusive alchemical, astrological, and numerical correspondences.',
    premium_benefit_3_title: 'Compatibility and Synastry',
    premium_benefit_3_desc: 'Understand the invisible threads of attraction, challenges, and affinities between your energy and the one you love.',
    premium_benefit_4_title: 'Dream Oracle',
    premium_benefit_4_desc: 'Unveil the hidden messages of your subconscious with guided interpretations and secure archiving.',
    premium_benefit_5_title: 'Unlimited Extra Charts',
    premium_benefit_5_desc: 'Create, save, and analyze birth charts and numerology reports of your friends and family.',
    select_plan_title: 'Choose your Alignment',
    select_plan_desc: 'Select the ideal cycle to continue your cosmic journey:',
    plan_annual_save: 'Save 33%',
    plan_annual_title: 'Annual Plan',
    plan_annual_sub: 'Billed annually',
    year: 'year',
    month: 'month',
    plan_monthly_title: 'Monthly Plan',
    plan_monthly_sub: 'Cancel anytime',
    plan_annual_desc_info: 'The annual cycle guarantees 12 full months of uninterrupted astrological tuning, daily transits, and AI for less than the price of a coffee per week.',
    plan_monthly_desc_info: 'The monthly cycle is billed automatically every 30 days. It offers full cancellation flexibility at any time directly from the dashboard.',
    btn_connecting_stripe: 'CONNECTING...',
    stripe_secure_payment_guaranteed: 'Stripe Secure Checkout • Immediate Activation',
    conversion_nasa_text: 'Advanced astrological interpretation technology, with high-precision astronomical calculations connected to the NASA/JPL software engine, calculating planetary positions, houses, and transits in real-time.'
  },
  es: {
    "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.": "Inspeccione las claves criptográficas locales de su carta astral mística y gestione la configuración de privacidad.",
    "Dados de Nascimento e Orbe": "Datos de Nacimiento y Orbe",
    "Dados atualizados com êxito! Recalculando coordenadas estelares...": "¡Datos actualizados con éxito! Recalculando coordenadas estelares...",
    "Data de Nascimento": "Fecha de Nacimiento",
    "Horário exato": "Hora exacta",
    "Atualizar e Recalcular Mapa": "Actualizar y Recalcular Carta",
    "Preferências Globais": "Preferencias Globales",
    "Idioma do Oráculo": "Idioma del Oráculo",
    "Autenticação Segura Local": "Autenticación Segura Local",
    "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.": "Sus diarios de sueños, oráculos de tarot y configuraciones se guardan únicamente en la caja de arena local del navegador del dispositivo para mantener absoluta privacidad.",

    currentLocation: 'Tu Configuración Natal',
    logout: 'Cerrar Sesión',
    unsupportedFirebase: 'Modo Local Activo: Su sesión y datos se almacenan de manera local y privada.',
    astroModelEngine: 'Motor Cósmico Interno: Precisión matemática sin conexión.',

    // Billing / Premium
    sub_status: 'Estado de la Suscripción',
    sub_expires: 'Fecha de Expiración',
    sub_billing_info: 'Facturación mediante Stripe Subscription',
    sub_manage: 'Gestionar Facturación',
    sub_created_at: 'Miembro desde',
    trial_remaining: 'Tu sintonización inicial gratuita está activa con acceso ilimitado.',
    sub_sim_pay_success: '¡Acceso Premium activado con éxito vía Stripe!',

    // Extra maps
    "Lista de Mapas Extras Cadastrados": "Lista de Cartas Extras Registradas",
    "Processando alinhamento estelar secundário...": "Procesando alineación estelar secundaria...",
    "Nenhum mapa extra cadastrado.": "No hay cartas extras registradas.",
    "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.": "Añade hasta 2 perfiles de amigos o familiares para comparar cartas astrológicas y sinastrias.",
    "Nome": "Nombre",
    "Cidade": "Ciudad",
    "Visualizar": "Visualizar",
    "Remover": "Eliminar",
    "Limites": "Límite de cartas extras alcanzado (máx. 2).",

    // Personal Protection and Edit Social Profile keys
    "Amuletos & Símbolos de Proteção Pessoais": "Amuletos y Símbolos de Protección Personales",
    "Seu Elemento Ativo: Ar": "Tu Elemento Activo: Aire",
    "Pedras de Filtro": "Piedras de Filtro",
    "Editar Dados do Perfil Social": "Editar Datos del Perfil Social",
    "Adicione suas mídias sociais e bio para os outros visualizadores.": "Agregue sus redes sociales y biografía para los demás espectadores.",
    "Bio Curta": "Biografía Corta",
    "Escreva algo curto sobre você...": "Escribe algo corto sobre ti...",
    "Instagram (@usuario)": "Instagram (@usuario)",
    "@seu_perfil": "@tu_perfil",
    "Facebook (usuário ou link)": "Facebook (usuario o enlace)",
    "link_do_facebook": "enlace_de_facebook",
    "Cancelar": "Cancelar",
    "Salvar Perfil": "Guardar Perfil",

    // Map Sync keys
    "MAPA SINCRONIZADO": "CARTA SINCRONIZADA",
    "Seu mapa astral principal foi protegido e sincronizado com sua conta.": "Tu carta astral principal ha sido protegida y sincronizada con tu cuenta.",
    "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.": "A partir de ahora recibirás actualizaciones continuas sobre tránsitos, ciclos, frecuencias energéticas y movimientos celestes relacionados con tu carta.",
    "SINCRONIZAÇÃO ATIVA": "SINCRONIZACIÓN ACTIVA",
    "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.": "Guarda tu carta para recibir actualizaciones personalizadas de tránsitos, energías, predicciones y movimientos astrológicos en tiempo real.",
    "SALVAR MAPA": "GUARDAR CARTA",
    premium_benefit_1_title: 'Análisis Avanzado de IA Orbia',
    premium_benefit_1_desc: 'Consejos diarios e interpretación personalizada en tiempo real basada en sus tránsitos y carta natal.',
    premium_benefit_2_title: 'Tarot Cósmico Profesional',
    premium_benefit_2_desc: 'Lecturas ilimitadas con correspondencias alquímicas, astrológicas y numéricas exclusivas.',
    premium_benefit_3_title: 'Compatibilidad y Sinastria',
    premium_benefit_3_desc: 'Entenda los hilos invisibles de atracción, desafíos y afinidades entre su energía y la de quien ama.',
    premium_benefit_4_title: 'Oráculo de los Sueños',
    premium_benefit_4_desc: 'Descubra las mensajes ocultas de su subconsciente con interpretaciones guiadas y archivo seguro.',
    premium_benefit_5_title: 'Cartas Extras Ilimitadas',
    premium_benefit_5_desc: 'Cree, guarde y analice las cartas natales y numerología de sus amigos y familiares sin límites.',
    select_plan_title: 'Elija su Alineación',
    select_plan_desc: 'Seleccione el ciclo ideal para continuar su viaje cósmico:',
    plan_annual_save: 'Ahorre 33%',
    plan_annual_title: 'Plan Anual',
    plan_annual_sub: 'Cobrado anualmente',
    year: 'año',
    month: 'mes',
    plan_monthly_title: 'Plan Mensual',
    plan_monthly_sub: 'Cancele cuando quiera',
    plan_annual_desc_info: 'El ciclo anual garantiza 12 meses completos de sintonía astrológica ininterrumpida, trânsitos diarios e IA por menos del precio de un café a la semana.',
    plan_monthly_desc_info: 'El ciclo mensual se cobra de forma automática cada 30 dias. Ofrece total flexibilidad de cancelación en cualquier momento desde el panel.',
    btn_connecting_stripe: 'CONECTANDO...',
    stripe_secure_payment_guaranteed: 'Stripe Secure Checkout • Activación Inmediata',
    conversion_nasa_text: 'Tecnología astrológica avanzada de interpretación, con cálculos astronómicos de altísima precisión conectados al motor de software de la NASA/JPL, que calcula posiciones planetarias, casas y tránsitos en tiempo real.'
  },
  de: {
    "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.": "Überprüfen Sie die lokalen kryptografischen Schlüssel Ihres mystischen Geburtshoroskops und verwalten Sie die Datenschutzeinstellungen.",
    "Dados de Nascimento e Orbe": "Geburtsdaten und Orbis",
    "Dados atualizados com êxito! Recalculando coordenadas estelares...": "Daten erfolgreich aktualisiert! Sternenkoordinaten werden neu berechnet...",
    "Data de Nascimento": "Geburtsdatum",
    "Horário exato": "Genaue Uhrzeit",
    "Atualizar e Recalcular Mapa": "Horoskop aktualisieren und neu berechnen",
    "Preferências Globais": "Globale Einstellungen",
    "Idioma do Oráculo": "Sprache des Orakels",
    "Autenticação Segura Local": "Sichere lokale Authentifizierung",
    "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.": "Ihre Traumtagebücher, Tarot-Orakel und Einstellungen werden ausschließlich in der lokalen Browser-Sandbox des Geräts gespeichert, um absolute Vertraulichkeit zu gewährleisten.",

    currentLocation: 'Ihre Natal-Koordinaten',
    logout: 'Sitzung beenden',
    unsupportedFirebase: 'Aktiver lokaler Modus: Ihre Sitzung und Daten werden lokal und privat gespeichert.',
    astroModelEngine: 'Interne kosmische Engine: Mathematische Präzision offline.',

    // Billing / Premium
    sub_status: 'Abonnement-Status',
    sub_expires: 'Ablaufdatum',
    sub_billing_info: 'Abrechnung über Stripe Subscription',
    sub_manage: 'Abrechnung Verwalten',
    sub_created_at: 'Mitglied seit',
    trial_remaining: 'Ihre kostenlose Einführungsphase ist aktiv mit uneingeschränktem Zugriff.',
    sub_sim_pay_success: 'Premium-Zugang erfolgreich über Stripe freigeschaltet!',

    // Extra maps
    "Lista de Mapas Extras Cadastrados": "Liste der registrierten Zusatzhoroskope",
    "Processando alinhamento estelar secundário...": "Verarbeitung der sekundären Sternenausrichtung...",
    "Nenhum mapa extra cadastrado.": "Keine Zusatzhoroskope registriert.",
    "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.": "Fügen Sie bis zu 2 Profile von Freunden oder Familie hinzu, um Horoskope und Synastrien zu vergleichen.",
    "Nome": "Name",
    "Cidade": "Stadt",
    "Visualizar": "Anzeigen",
    "Remover": "Entfernen",
    "Limites": "Limit für Zusatzhoroskope erreicht (max. 2).",

    // Personal Protection and Edit Social Profile keys
    "Amuletos & Símbolos de Proteção Pessoais": "Persönliche Schutzamulette & Symbole",
    "Seu Elemento Ativo: Ar": "Dein aktives Element: Luft",
    "Pedras de Filtro": "Filtersteine",
    "Editar Dados do Perfil Social": "Sozialprofildaten bearbeiten",
    "Adicione suas mídias sociais e bio para os outros visualizadores.": "Fügen Sie Ihre sozialen Medien und Ihre Biografie für andere Betrachter hinzu.",
    "Bio Curta": "Kurze Biografie",
    "Escreva algo curto sobre você...": "Schreiben Sie etwas Kurzes über sich selbst...",
    "Instagram (@usuario)": "Instagram (@benutzername)",
    "@seu_perfil": "@dein_profil",
    "Facebook (usuário ou link)": "Facebook (Benutzername oder Link)",
    "link_do_facebook": "facebook_link",
    "Cancelar": "Abbrechen",
    "Salvar Perfil": "Profil speichern",

    // Map Sync keys
    "MAPA SINCRONIZADO": "HOROSKOP SYNCHRONISIERT",
    "Seu mapa astral principal foi protegido e sincronizado com sua conta.": "Ihr Hauptgeburtshoroskop wurde geschützt und mit Ihrem Konto synchronisiert.",
    "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.": "Ab sofort erhalten Sie kontinuierliche Updates über Transite, Zyklen, Energiefrequenzen und Himmelsbewegungen im Zusammenhang mit Ihrem Horoskop.",
    "SINCRONIZAÇÃO ATIVA": "AKTIVE SYNCHRONISIERUNG",
    "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.": "Speichern Sie Ihr Horoskop, um personalisierte Updates zu Transiten, Energien, Vorhersagen und astrologischen Bewegungen in Echtzeit zu erhalten.",
    "SALVAR MAPA": "HOROSKOP SPEICHERN",
    premium_benefit_1_title: 'Erweiterte Orbia-KI-Analyse',
    premium_benefit_1_desc: 'Tägliche Beratung und personalisierte Echtzeit-Deutung basierend auf Ihren Transiten und Ihrem Geburtshoroskop.',
    premium_benefit_2_title: 'Professionelles Kosmisches Tarot',
    premium_benefit_2_desc: 'Unbegrenzte Legungen mit exklusiven alchemistischen, astrologischen und numerischen Entsprechungen.',
    premium_benefit_3_title: 'Kompatibilität und Synastrie',
    premium_benefit_3_desc: 'Verstehen Sie die unsichtbaren Fäden der Anziehung, Herausforderungen und Affinitäten zwischen Ihrer Energie und der Ihrer Liebsten.',
    premium_benefit_4_title: 'Traumorakel',
    premium_benefit_4_desc: 'Enthüllen Sie die verborgenen Botschaften Ihres Unterbewusstseins mit geführten Deutungen und sicherer Archivierung.',
    premium_benefit_5_title: 'Unbegrenzte Zusatzhoroskope',
    premium_benefit_5_desc: 'Erstellen, speichern und analysieren Sie Geburtshoroskope und Numerologieberichte von Freunden und Familie ohne Einschränkung.',
    select_plan_title: 'Wählen Sie Ihre Ausrichtung',
    select_plan_desc: 'Wählen Sie den idealen Zyklus, um Ihre kosmische Reise fortzusetzen:',
    plan_annual_save: '33% Sparen',
    plan_annual_title: 'Jahresabo',
    plan_annual_sub: 'Jährlich abgerechnet',
    year: 'Jahr',
    month: 'Monat',
    plan_monthly_title: 'Monatsabo',
    plan_monthly_sub: 'Jederzeit kündbar',
    plan_annual_desc_info: 'Das Jahresabo garantiert 12 volle Monate ununterbrochene astrologische Abstimmung, tägliche Transite und KI für weniger als den Preis eines Kaffees pro Woche.',
    plan_monthly_desc_info: 'Das Monatsabo wird alle 30 Tage automatisch abgerechnet. Es bietet jederzeit die volle Flexibilität zur Kündigung direkt im Dashboard.',
    btn_connecting_stripe: 'VERBINDEN...',
    stripe_secure_payment_guaranteed: 'Stripe Secure Checkout • Sofortige Aktivierung',
    conversion_nasa_text: 'Fortschrittliche astrologische Deutungstechnologie, mit hochpräzisen astronomischen Berechnungen, die mit der NASA/JPL-Software-Engine verbunden sind, um Planetenpositionen, Häuser und Transite in Echtzeit zu berechnen.'
  },
  fr: {
    "Inspecione as chaves criptográficas locais de seu mapa astral místico e gerencie as configurações de privacidade.": "Inspectez les clés cryptographiques locales de votre carte du ciel mystique et gérez les paramètres de confidentialité.",
    "Dados de Nascimento e Orbe": "Données de Naissance et Orbe",
    "Dados atualizados com êxito! Recalculando coordenadas estelares...": "Données mises à jour avec succès ! Recalcul des coordonnées stellaires...",
    "Data de Nascimento": "Date de Naissance",
    "Horário exato": "Heure exacte",
    "Atualizar e Recalcular Mapa": "Mettre à jour & Recalculer la Carte",
    "Preferências Globais": "Préférences Globales",
    "Idioma do Oráculo": "Langue de l'Oracle",
    "Autenticação Segura Local": "Authentification Locale Sécurisée",
    "Seus diários de sonhos, oráculos de tarot e configurações são guardadas unicamente na caixa sandbox local do navegador do dispositivo para manter sigilo absoluto.": "Vos journaux de rêves, oracles de tarot et configurations sont enregistrés uniquement dans le bac à sable (sandbox) local de votre navigateur pour maintenir une confidentialité absolue.",

    currentLocation: 'Vos Coordonnées de Naissance',
    logout: 'Déconnexion',
    unsupportedFirebase: 'Mode Local Actif : Votre session et vos données sont stockées de manière locale et privée.',
    astroModelEngine: 'Moteur Cosmique Interne : Précision mathématique hors ligne.',

    // Billing / Premium
    sub_status: "Statut de l'Abonnement",
    sub_expires: "Date d'Expiration",
    sub_billing_info: 'Facturation via Stripe Subscription',
    sub_manage: 'Gérer la Facturation',
    sub_created_at: 'Membre depuis',
    trial_remaining: 'Votre synchronisation gratuite initiale est pleinement active avec un accès illimité.',
    sub_sim_pay_success: 'Accès Premium activé avec succès via Stripe !',

    // Extra maps
    "Lista de Mapas Extras Cadastrados": "Liste des Cartes Extras Enregistrées",
    "Processando alinhamento estelar secundário...": "Traitement de l'alignement stellaire secondaire...",
    "Nenhum mapa extra cadastrado.": "Aucune carte extra enregistrée.",
    "Adicione até 2 perfis de amigos ou familiares para comparar as cartas astrológicas e sinastrias.": "Ajoutez jusqu'à 2 profils d'amis ou de famille pour comparer les thèmes astrologiques et les synastries.",
    "Nome": "Nom",
    "Cidade": "Ville",
    "Visualizar": "Visualiser",
    "Remover": "Supprimer",
    "Limites": "Limite de cartes extras atteinte (max 2).",

    // Personal Protection and Edit Social Profile keys
    "Amuletos & Símbolos de Proteção Pessoais": "Amulettes et Symboles de Protection Personnels",
    "Seu Elemento Ativo: Ar": "Votre Élément Actif : Air",
    "Pedras de Filtro": "Pierres de Filtre",
    "Editar Dados do Perfil Social": "Modifier les Données du Profil Social",
    "Adicione suas mídias sociais e bio para os outros visualizadores.": "Ajoutez vos réseaux sociaux et votre biographie pour les autres visiteurs.",
    "Bio Curta": "Biographie Courte",
    "Escreva algo curto sobre você...": "Écrivez quelque chose de court sur vous...",
    "Instagram (@usuario)": "Instagram (@utilisateur)",
    "@seu_perfil": "@votre_profil",
    "Facebook (usuário ou link)": "Facebook (utilisateur ou lien)",
    "link_do_facebook": "lien_facebook",
    "Cancelar": "Annuler",
    "Salvar Perfil": "Enregistrer le profil",

    // Map Sync keys
    "MAPA SINCRONIZADO": "CARTE SYNCHRONISÉE",
    "Seu mapa astral principal foi protegido e sincronizado com sua conta.": "Votre carte du ciel principale a été protégée et synchronisée avec votre compte.",
    "A partir de agora você receberá atualizações contínuas sobre trânsitos, ciclos, frequências energéticas e movimentações celestes relacionadas ao seu mapa.": "À partir de maintenant, vous recevrez des thèmes mis à jour continuellement sur les transits, les cycles, les fréquences énergétiques et les mouvements célestes liés à votre carte.",
    "SINCRONIZAÇÃO ATIVA": "SYNCHRONISATION ACTIVE",
    "Salve seu mapa para receber atualizações personalizadas de trânsitos, energias, previsões e movimentos astrológicos em tempo real.": "Enregistrez votre carte pour recevoir des mises à jour personnalisées sur les transits, les énergies, les prévisions et les mouvements astrologiques en temps réel.",
    "SALVAR MAPA": "ENREGISTRER LA CARTE",
    premium_benefit_1_title: 'Analyse Avancée Orbia IA',
    premium_benefit_1_desc: 'Conseils quotidiens et interprétation personnalisée en temps réel basée sur vos transits et votre thème natal.',
    premium_benefit_2_title: 'Tarot Cosmic Professionnel',
    premium_benefit_2_desc: 'Tirages illimités avec correspondances alchimiques, astrologiques et numériques exclusives.',
    premium_benefit_3_title: 'Compatibilité et Synastrie',
    premium_benefit_3_desc: 'Comprenez les fils invisibles de l\'attraction, des défis et des affinités entre votre énergie et celle de ceux que vous aimez.',
    premium_benefit_4_title: 'Oracle des Rêves',
    premium_benefit_4_desc: 'Dévoilez les messages cachés de votre subconscient avec des interprétations guidées et un archivage sécurisé.',
    premium_benefit_5_title: 'Thèmes Extras Illimités',
    premium_benefit_5_desc: 'Créez, enregistrez et analysez les cartes du ciel et les rapports de numérologie de vos amis et de votre famille.',
    select_plan_title: 'Choisissez votre Alignement',
    select_plan_desc: 'Sélectionnez le cycle idéal pour continuer votre voyage cosmique :',
    plan_annual_save: 'Économisez 33%',
    plan_annual_title: 'Plan Annuel',
    plan_annual_sub: 'Facturé annuellement',
    year: 'an',
    month: 'mois',
    plan_monthly_title: 'Plan Mensuel',
    plan_monthly_sub: 'Annulez quand vous voulez',
    plan_annual_desc_info: 'Le cycle annuel garantit 12 mois complets de syntonisation astrologique ininterrompue, de transits quotidiens et d\'IA pour moins que le prix d\'un café par semaine.',
    plan_monthly_desc_info: 'Le cycle mensuel est facturé automatiquement tous les 30 jours. Il offre une flexibilité totale d\'annulation à tout moment directement depuis le tableau de bord.',
    btn_connecting_stripe: 'CONNEXION...',
    stripe_secure_payment_guaranteed: 'Paiement Sécurisé Stripe • Activation Immédiate',
    conversion_nasa_text: 'Technologie avancée d\'interprétation astrologique, avec des calculs astronomiques de haute précision connectés au moteur logiciel de la NASA/JPL, calculant les positions planétaires, les maisons et les transits en temps réel.'
  }
};
