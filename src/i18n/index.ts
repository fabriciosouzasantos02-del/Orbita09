...
export const mergedTranslations: Record<Language, Record<string, string>> = { pt: {}, en: {}, es: {}, de: {}, fr: {} };
...
try { applyTranslationPatches(mergedTranslations); } catch (e) { console.warn('Note: applyTranslationPatches deferred or already merged.'); }
...
export function verifyTranslationKeys(): string[] {
  ...
  const errors: string[] = [];
  for (const key of ptKeys) for (const lang of languages) if (!mergedTranslations[lang][key]) errors.push(`Missing translation key: "${key}" in language "${lang}"`);
  return errors;
}